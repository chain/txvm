...
func usage() {
	opts := []string{
		...
		"verify [-s] PUBHEX SIGHEX <message",
	}
	fmt.Println("Usage:")
	for _, o := range opts {
		fmt.Printf("\t%s %s\n", os.Args[0], o)
	}
	fmt.Println("PRIVHEX is a hex-encoded private key.")
	fmt.Println("PUBHEX is a hex-encoded public key.")
	fmt.Println("SIGHEX is a hex-encoded signature.")
