...
// ScMulCofactor computes 8*p, where p is the ed25519 point and 8 is a cofactor,
...
func (z *Point) ScMulCofactor(p *Point) *Point {
	...
	(*edwards25519.ExtendedGroupElement)(p).Double(&c)
	c.ToProjective(&pproj)
	pproj.Double(&c)
	c.ToProjective(&pproj)
	pproj.Double(&c)
	c.ToExtended((*edwards25519.ExtendedGroupElement)(z))
	return z
}
